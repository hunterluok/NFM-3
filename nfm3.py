...
from utils import construce_alpa
...
class NeuralFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            self.FM = 0.5 * tf.subtract(self.summed_features_emb_square, self.squared_sum_features_emb)
            
            # when degree=3.  2+3 
            sqrd = tf.pow(self.summed_features_emb,3)
            sqrd_2_1 = -3 * tf.multiply(self.squared_sum_features_emb,self.summed_features_emb)
            sqrd_3  = tf.pow(nonzero_embeddings,3)
            sqrd_3_sum = 2 * tf.reduce_sum(sqrd_3,1)
            self.FM_3 = 1./6 * tf.add_n([sqrd,sqrd_2_1,sqrd_3_sum])
            self.FM = tf.add(self.FM,self.FM_3)

            """
            self.FM = 0.
            d = 3
            a = construce_alpa(3).get_bet_k
            a.dtype='float32'
            
            def cal_concat(d):
                constant = 1.
                for i in range(1,d+1):
                    constant =  tf.multiply(constant,i)
                return 1./constant
            #constant = cal_concat(d)
            constant= construce_alpa(3).constant
            
            for k in np.arange(1,d+1):
                pow_sum = tf.reduce_sum(tf.pow(nonzero_embeddings,k),1)
                sum_pow = tf.pow(tf.reduce_sum(nonzero_embeddings,1),d-k)
                beta_k = a[k-1]
                temp = tf.multiply(pow_sum,sum_pow)
                temp = tf.multiply(beta_k,temp)
                self.FM = tf.add(self.FM,temp)
            self.FM = tf.multiply(constant,self.FM)
            """
